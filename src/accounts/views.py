...
def otp_login():
    ...
    if form.validate_on_submit():
        if request.form.get('action') == 'send_otp':
            ...
            utc_now = datetime.utcnow().replace(tzinfo=pytz.utc)
            if otp_entry and utc_now <= otp_entry.expires_at.replace(tzinfo=pytz.utc):
...
def update_failed_attempts(email):
    ...
    if user:
        ...
        # Reset failed attempts if the last attempt was more than an hour ago
        if not user.last_failed_login or utc_now - user.last_failed_login.replace(tzinfo=pytz.UTC) >= timedelta(hours=1):
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            if user.last_failed_login and utc_now - user.last_failed_login.replace(tzinfo=pytz.UTC) < timedelta(hours=1) and user.failed_login_attempts >= 5:
...
def confirm_email(token):
    ...
    if user.email == email:
        ...
        user.confirmed_on = datetime.utcnow().replace(tzinfo=pytz.utc)
        db.session.add(user)
        db.session.commit()
