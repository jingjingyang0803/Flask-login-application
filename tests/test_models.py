    ...
    def test_user_registration(self):
        ...
        with self.client:
            ...
            self.client.post(
                "/register",
                data=dict(
                    email="jingjing.yang@tuni.fi", password="Test_user123", confirm="Test_user123"
                ),
                follow_redirects=True,
            )
            user = User.query.filter_by(email="jingjing.yang@tuni.fi").first()
            self.assertTrue(user.id)
            self.assertTrue(user.email == "jingjing.yang@tuni.fi")
